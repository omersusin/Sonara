...
import com.sonara.app.ui.screens.settings.DarkThemeScreen
import com.sonara.app.ui.screens.settings.LookAndFeelScreen
...
sealed class Screen(val route: String, val label: String) {
    ...
    data object SettingsLookAndFeel : Screen("look_and_feel", "Look & Feel")
    data object SettingsDarkTheme : Screen("dark_theme", "Dark theme")
    ...
    data object AlbumDetail : Screen("album_detail/{name}/{artist}/{plays}/{imageUrl}", "Album") {
        fun createRoute(name: String, artist: String, plays: String, imageUrl: String) =
            "album_detail/${java.net.URLEncoder.encode(name, "UTF-8")}/${
            ...
            }/${java.net.URLEncoder.encode(imageUrl.ifBlank { "-" }, "UTF-8")}"
    }
    ...
    data object ArtistDetail : Screen("artist_detail/{name}", "Artist") {
        fun createRoute(name: String, imageUrl: String = "") =
            "artist_detail/${java.net.URLEncoder.encode(name, "UTF-8")}" +
                    if (imageUrl.isNotBlank()) "?initialImageUrl=${
                        java.net.URLEncoder.encode(
                            imageUrl,
                            "UTF-8"
                        )
    ...
    data object TrackDetail : Screen("track_detail/{title}/{artist}", "Track") {
        fun createRoute(title: String, artist: String) = "track_detail/${
            java.net.URLEncoder.encode(
                title,
                "UTF-8"
            )
    ...
    data object ArtistDiscography : Screen("artist_discography/{name}", "Discography") {
        fun createRoute(name: String) =
            "artist_discography/${java.net.URLEncoder.encode(name, "UTF-8")}"
    }

    data object SimilarArtists : Screen("similar_artists/{name}", "Similar Artists") {
        fun createRoute(name: String) =
            "similar_artists/${java.net.URLEncoder.encode(name, "UTF-8")}"
    }
    ...
    data object TrackScrobbleHistory :
        Screen("track_scrobble_history/{title}/{artist}", "Scrobble History") {
        fun createRoute(title: String, artist: String) =
            "track_scrobble_history/${
                java.net.URLEncoder.encode(
                    title,
                    "UTF-8"
                )
    ...
    data object FriendProfile :
        Screen("friend_profile/{username}/{realname}/{playcount}/{avatarUrl}", "Friend Profile") {
        fun createRoute(username: String, realname: String, playcount: String, avatarUrl: String) =
            ...
            }/${java.net.URLEncoder.encode(avatarUrl.ifBlank { "-" }, "UTF-8")}"
    }
}
...
fun SonaraNavigation() {
    ...
    Scaffold(
        ...
        bottomBar = { if (!hideBottomBar) SonaraBottomBar(navController) }) { padding ->
        ...
        NavHost(
            ...
            composable(Screen.Insights.route) {
                InsightsScreen(
                    ...
                    onFriendClick = { f ->
                        navController.navigate(
                            Screen.FriendProfile.createRoute(
                                f.name,
                                f.realname,
                                f.playcount,
                                f.imageUrl
            ...
            composable(Screen.SettingsLookAndFeel.route) {
                LookAndFeelScreen(
                    onBackClick = { navController.popBackStack() },
                    onNavigateDarkThemeScreen = { navController.navigate(Screen.SettingsDarkTheme.route) })
            }
            composable(Screen.SettingsDarkTheme.route) { DarkThemeScreen(onBackClick = { navController.popBackStack() }) }
            ...
            composable(Screen.AlbumDetail.route) { entry ->
                ...
                val artist =
                    java.net.URLDecoder.decode(
                        entry.arguments?.getString("artist") ?: "",
                        "UTF-8"
                    )
                val plays =
                    java.net.URLDecoder.decode(
                        entry.arguments?.getString("plays") ?: "",
                        "UTF-8"
                    )
                        .let { if (it == "-") "" else it }
                val imageUrl = java.net.URLDecoder.decode(
            ...
            composable(
                ...
                val trackTitle = java.net.URLDecoder.decode(
                    entry.arguments?.getString("trackTitle") ?: "",
                    "UTF-8"
                )
                val initialImageUrl = java.net.URLDecoder.decode(
                    entry.arguments?.getString("initialImageUrl") ?: "",
                    "UTF-8"
                )
                ArtistDetailScreen(
                    artistName = name,
                    ...
                    onSeeAllDiscography = { a ->
                        navController.navigate(
                            Screen.ArtistDiscography.createRoute(
                                a
                            )
                        )
                    },
                    onSeeAllSimilar = { a ->
                        navController.navigate(
                            Screen.SimilarArtists.createRoute(
                                a
                            )
                        )
                    }
            ...
            composable(Screen.TrackDetail.route) { entry ->
                ...
                TrackDetailScreen(
                    ...
                    onArtistClick = { name ->
                        navController.navigate(
                            Screen.ArtistDetail.createRouteWithTrack(
                                name,
                                title
                            )
                        )
                    ...
                    onSeeAllScrobbles = {
                        navController.navigate(
                            Screen.TrackScrobbleHistory.createRoute(
                                title,
                                artist
                            )
                        )
            ...
            composable("collage") {
                val vm: com.sonara.app.ui.screens.insights.InsightsViewModel =
            ...
            composable(
                ...
                val realname = java.net.URLDecoder.decode(
                    entry.arguments?.getString("realname") ?: "",
                    "UTF-8"
                ).let { if (it == "-") "" else it }
                val playcount = java.net.URLDecoder.decode(
                    entry.arguments?.getString("playcount") ?: "",
                    "UTF-8"
                ).let { if (it == "-") "0" else it }
                val avatarUrl = java.net.URLDecoder.decode(
                    entry.arguments?.getString("avatarUrl") ?: "",
                    "UTF-8"
                ).let { if (it == "-") "" else it }
                FriendProfileScreen(
                    username = username,
