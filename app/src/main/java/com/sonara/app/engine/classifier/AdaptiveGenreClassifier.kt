...
class AdaptiveGenreClassifier(private val context: Context) {
    ...
    fun classify(title: String, artist: String, album: String = ""): Pair<String, Float> {
        ...
        for ((g, c) in centroids) {
            val sim = cosine(feat, c.weights)
            if (sim > bestSim) { second = bestSim; bestSim = sim; best = g }
    ...
    fun train(genre: String, title: String, artist: String, album: String = "", weight: Float = 1f) {
        val g = normalize(genre)
        val feat = features(title, artist, album)
        val c = centroids.getOrPut(g) { Centroid(g, FloatArray(VOCAB), 0, 0f) }

        // Learning rate decay using Math.pow (Java stdlib, always available)
        val decayFactor = Math.pow(0.995, (totalSamples / 50).toDouble()).toFloat()
        val lr = weight * (0.3f * decayFactor).coerceAtLeast(0.01f)

        val old = c.totalWeight
        val nw = old + lr
        if (nw > 0) {
            for (i in c.weights.indices) {
                c.weights[i] = (c.weights[i] * old + feat[i] * lr) / nw
            }
        }
        c.samples++
        c.totalWeight = nw
    ...
    fun correct(wrong: String, correct: String, title: String, artist: String, album: String = "") {
        val feat = features(title, artist, album)
        centroids[normalize(wrong)]?.let { c ->
            val pw = -0.5f * 0.3f
            val nt = c.totalWeight + pw
            if (nt > 0.1f) {
                for (i in c.weights.indices) c.weights[i] = (c.weights[i] * c.totalWeight + feat[i] * pw) / nt
                c.totalWeight = nt
            }
        }
    ...
    fun getStats(): Map<String, Any> = mapOf(
        "total" to totalSamples,
        "genres" to centroids.size,
        "details" to centroids.map { (g, c) -> "$g:${c.samples}" }
    )

    fun save() {
        try {
            val data = mapOf(
                "centroids" to centroids.mapValues { (_, c) ->
                    mapOf("genre" to c.genre, "weights" to c.weights.toList(), "samples" to c.samples, "totalWeight" to c.totalWeight)
                },
    ...
    private fun load() {
        try {
            ...
            for ((g, v) in cData) {
                ...
                centroids[g.toString()] = Centroid(
                    g.toString(), wList.toFloatArray(),
                    (m["samples"] as? Double)?.toInt() ?: 0,
                    (m["totalWeight"] as? Double)?.toFloat() ?: 0f
                )
            }
    ...
    private fun features(title: String, artist: String, album: String): FloatArray {
        ...
        val text = "$artist $title $album".lowercase().trim()
        if (text.length < NGRAM) {
            for (c in text) out[(c.code * 31 % VOCAB + VOCAB) % VOCAB] += 1f
        } else {
            for (i in 0..text.length - NGRAM) {
                val h = text.substring(i, i + NGRAM).hashCode()
                out[(h % VOCAB + VOCAB) % VOCAB] += 1f
            }
        }
        val words = text.split(Regex("\\s+"))
        for (w in words) {
            if (w.length >= 2) out[(("W_$w").hashCode() % VOCAB + VOCAB) % VOCAB] += 2f
        }
        val norm = sqrt(out.sumOf { (it * it).toDouble() }).toFloat()
    ...
    private fun cosine(a: FloatArray, b: FloatArray): Float {
        ...
        val d = sqrt(na.toDouble()).toFloat() * sqrt(nb.toDouble()).toFloat()
        return if (d > 0) dot / d else 0f
    }

    private fun normalize(g: String): String = g.lowercase().trim().let {
        when {
            it.contains("hip") && it.contains("hop") -> "hip-hop"
            it.contains("r&b") || it.contains("rnb") -> "r&b"
            it.contains("electro") || it == "edm" || it == "house" || it == "techno" -> "electronic"
            it.contains("metal") -> "metal"
            it.contains("rock") || it == "alternative" -> "rock"
            it.contains("pop") -> "pop"
            it == "classical" || it == "orchestra" -> "classical"
            it.contains("jazz") -> "jazz"
            it.contains("blues") -> "blues"
            it.contains("reggae") -> "reggae"
            it.contains("latin") -> "latin"
            it == "country" -> "country"
            it == "soul" || it == "funk" -> "soul"
            else -> it
        }
    }
}
