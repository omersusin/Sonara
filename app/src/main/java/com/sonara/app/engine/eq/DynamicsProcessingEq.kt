...
class DynamicsProcessingEq private constructor(
        ...
        fun create(sessionId: Int = 0, priority: Int = Int.MAX_VALUE, frequencies: FloatArray = FREQUENCIES_10): DynamicsProcessingEq? {
            return try {
                ...
                val config = DynamicsProcessing.Config.Builder(
                    DynamicsProcessing.VARIANT_FAVOR_FREQUENCY_RESOLUTION, 1, false, 0, false, 0, true, bc, false
                ).apply {
                    for (i in 0 until bc) setPostEqBandAllChannelsTo(i, DynamicsProcessing.EqBand(true, frequencies[i], 0f))
                    setPostEqAllChannelsTo(DynamicsProcessing.Eq(true, true, bc))
                }.build()
                val dp = DynamicsProcessing(priority, sessionId, config)
                dp.enabled = true
                Log.i(TAG, "Created session=$sessionId bands=$bc enabled=${dp.enabled}")
                DynamicsProcessingEq(dp, bc, frequencies)
            } catch (e: Exception) { Log.e(TAG, "Create failed: ${e.message}"); null }
        }
    }

    val isEnabled: Boolean get() = dp.enabled
    ...
    fun setBandGain(band: Int, gainDb: Float) {
        ...
        catch (e: Exception) { Log.e(TAG, "setBandGain($band): ${e.message}") }
    }

    fun setAllBands(gainsDb: FloatArray) {
        if (gainsDb.size != bandCount) {
            val interp = interpolate(gainsDb, bandCount); interp.forEachIndexed { i, g -> setBandGain(i, g) }; return
        }
        gainsDb.forEachIndexed { i, g -> setBandGain(i, g) }
    }
