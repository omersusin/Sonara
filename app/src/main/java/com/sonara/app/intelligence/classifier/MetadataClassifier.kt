...
class MetadataClassifier {
    ...
    private val artistPatterns = listOf(
        ...
        Regex("deadmau5|skrillex|avicii|marshmello|tiesto|armin van buuren|martin garrix|david guetta|calvin harris|daft punk|zedd|kygo|alan walker|the chainsmokers|swedish house mafia|flume|odesza|disclosure|diplo|illenium", RegexOption.IGNORE_CASE) to listOf(GenreProb(Genre.ELECTRONIC, 0.85f)),
        ...
        Regex("taylor swift|ariana grande|ed sheeran|billie eilish|dua lipa|the weeknd|justin bieber|selena gomez|shawn mendes|halsey|lizzo|doja cat|olivia rodrigo|harry styles|adele|sia|charlie puth|bruno mars|lady gaga|katy perry|rihanna|bts|blackpink|twice|stray kids|aespa|newjeans|le sserafim|ive|seventeen|txt|enhypen|ateez|tarkan|hadise|aleyna tilki|mabel matiz|coldplay|imagine dragons|maroon 5|one direction", RegexOption.IGNORE_CASE) to listOf(GenreProb(Genre.POP, 0.85f)),
        Regex("led zeppelin|pink floyd|queen|ac/dc|the beatles|rolling stones|nirvana|foo fighters|radiohead|u2|muse|arctic monkeys|linkin park|green day|red hot chili|oasis|pearl jam|twenty one pilots|the killers|duman|manga|mor ve ötesi|barış manço|cem karaca", RegexOption.IGNORE_CASE) to listOf(GenreProb(Genre.ROCK, 0.85f)),
